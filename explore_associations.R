wd <- getwd()
setwd("Accessory code/Active-inactive-essential gold calls and analysis")
source("Clean_matt_calls.R")
setwd(wd)
setwd("Figure 1")
source("code.R")
setwd(wd)
...
is.complete <- function(expname) !(get_chipname(expname) %in% nonCompleteExpts)
...
get.counts <- function(feature) {
  inactive <- ifelse(length(offexps)==0, 0, sum(sapply(offexps, function(offexp) {
    feature %in% efd[efd$experiment_name==get_chipname(offexp),]$feature_name
  })))
  active <- ifelse(length(onexps)==0, 0, sum(sapply(onexps, function(onexp) {
    feature %in% efd[efd$experiment_name==get_chipname(onexp),]$feature_name
  })))
  unknown <- ifelse(length(unkexps)==0, 0, sum(sapply(unkexps, function(unkexp) {
    feature %in% efd[efd$experiment_name==get_chipname(unkexp),]$feature_name
  })))
...
all.tests <- function(pegnum, aijs) {
  offexps <<- colnames(remove_CEL_gz(eijs))[aijs[gene(pegnum),]<0.2]
  onexps <<- colnames(remove_CEL_gz(eijs))[aijs[gene(pegnum),]>0.8]
  unkexps <<- colnames(remove_CEL_gz(eijs))[aijs[gene(pegnum),]>=0.2 & aijs[gene(pegnum),]<=0.8]
